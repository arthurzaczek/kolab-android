...
public class SettingsView extends Activity implements Runnable
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
					...
					boolean isChecked)
			{
				...
				if (isChecked && "143".equals(txtPort.getText().toString()))
				{
					txtPort.setText("993");
						...
						&& "993".equals(txtPort.getText().toString()))
				{
					txtPort.setText("143");
				}
			}
		...
		updateMinSyncIntervalSpinnter(spMinSyncInterval,
				pref.getMinSynIntervalMinutes());
	...
	private class Minute
	{
		...
		public Minute(int v, String t)
		{
			this.Value = v;
			this.Text = t;
		}

		public String	Text	= "Disabled";
		public int		Value	= 0;
	...
	private void updateMinSyncIntervalSpinnter(Spinner sp,
			int i)
	{
		ArrayAdapter<Minute> adapter = new ArrayAdapter<Minute>(
				SettingsView.this, android.R.layout.simple_spinner_item,
				new Minute[] { new Minute(0, "disabled"),
						new Minute(15, "15 Minutes"),
						new Minute(30, "30 Minutes"),
						new Minute(60, "1 Hour"),
						new Minute(120, "2 Hours"),
						new Minute(180, "3 Hours"),
						new Minute(360, "6 Hours")});
	 ...
	 * determines currently selected item from spinner, updates spinner's list
	 * of items and tries to set the old value again. If imapFolders is empty
	 * and a default value has been set, this will be used instead.
	 */
		...
		else
		{
			if (selectedItem != null)
			{
				adapter = new ArrayAdapter<String>(
						...
						new String[] { selectedItem.toString() });
				newPosition = 0;
			}
			else
			{
	...
	public void onClick(View v)
	{
		...
		{
			showMsgDialog(
					R.string.checkSettingsTestFailedUnknownExceptionTitle,
					R.string.checkSettingsTestFailedUnknownException);
			Log.e(LOG_TAG, "unknown exception in onClick():", e);
		}
	}

	private void showMsgDialog(final int msgResIdTitle, final int msgResIdMsg,
			final Object... args)
	{
		mHandler.post(new Runnable() {
			public void run()
			{
				...
				new AlertDialog.Builder(SettingsView.this)
						...
						.setCancelable(true)
						.setNeutralButton(
								getString(R.string.checkSettingsMsgDialogButton),
								null).show();
			}
		});
	}
	...
	public void run()
	{
		...
		try
		{
			...
			Session session = ImapClient.getDefaultImapSession(
					Integer.parseInt(port), cbUseSSL.isChecked());
			store = ImapClient.openServer(session, hostname, username,
					txtPassword.getText().toString());
			Log.d(LOG_TAG, "Authentication with user " + username + " at "
					+ hostname + ':' + port + " has been successful.");
		...
		catch (final MessagingException e)
		{
			...
			{
				showMsgDialog(
						R.string.checkSettingsTestFailedUnknownHostExceptionTitle,
						R.string.checkSettingsTestFailedUnknownHostException,
						hostname);
				Log.e(LOG_TAG, "Host is unresolved: " + hostname + ':' + port,
						e);
			...
			{
				showMsgDialog(
						R.string.checkSettingsTestFailedConnectExceptionTitle,
						R.string.checkSettingsTestFailedConnectException,
						hostname);
				Log.e(LOG_TAG, "Failed to connecto to host: " + hostname + ':'
						+ port, e);
			...
			{
				showMsgDialog(
						R.string.checkSettingsTestFailedAuthenticationFailedExceptionTitle,
						R.string.checkSettingsTestFailedAuthenticationFailedException,
						hostname);
				Log.e(LOG_TAG, "Authentication failed with user " + username
						+ " at " + hostname + ':' + port, e);
			...
			{
				showMsgDialog(
						R.string.checkSettingsTestFailedFolderNotFoundExceptionTitle,
						R.string.checkSettingsTestFailedFolderNotFoundException,
						hostname);
				Log.e(LOG_TAG, "Given namespace '" + namespace
						+ "' seems to be wrong:", e);
			...
			else
			{
				showMsgDialog(
						...
						hostname);
				Log.e(LOG_TAG,
						"Unknown messaging exception while connecting to "
								+ hostname + ':' + port + " with user "
								+ username, e);
			}
		}
		catch (Exception e)
		{
			...
			Log.e(LOG_TAG, "Unknown exception while connecting to " + hostname
					+ ':' + port + " with user " + username, e);
		}
		try
		{
		...
		catch (final MessagingException e)
		{
			showMsgDialog(
					R.string.checkSettingsTestFailedUnknownExceptionTitle,
					R.string.checkSettingsTestFailedUnknownException, hostname);
			Log.e(LOG_TAG, "Unknown exception while closing connection to "
					+ hostname + ':' + port, e);
		}
		mProgDialog.dismiss();
	}
	...
	private void showAcceptKeyDialog(final Object... args)
	{
		mHandler.post(new Runnable() {
			public void run()
			{
				...
				final java.security.cert.X509Certificate[] chain = TrustManagerFactory
						.getLastUsedChain();
				...
				new AlertDialog.Builder(SettingsView.this)
						.setTitle(
								getString(R.string.checkSettingsTestFailedInvalidCertificateTitle))
						.setMessage(
								getString(R.string.checkSettingsTestFailedInvalidCertificateMessage)
										+ exMessage
										+ ":\n"
										+ chainInfo.toString())
						.setCancelable(true)
								...
								new DialogInterface.OnClickListener() {
											...
											int which)
									{
										try
										{
											...
											TrustManagerFactory
													.addCertificateChainToKeystore(
															getApplicationContext(),
															chain);
											Log.d(LOG_TAG,
													"certificate saved to keystore");
											showMsgDialog(
													R.string.checkSettingsCertificateChainSavedTitle,
													R.string.checkSettingsCertificateChainSaved);
											// username and password have not
											// been checked yet ==> restart test
											onClickCheckSettings();
										}
										catch (java.security.cert.CertificateException e)
										{
											Log.e(LOG_TAG,
													"Adding certificate chain to local keystore failed: ",
													e);
											showMsgDialog(
													R.string.checkSettingsTestFailedKeystoreErrorTitle,
						...
						.setNegativeButton(
								...
								new DialogInterface.OnClickListener() {
									...
									{
										Log.d(LOG_TAG,
												"User declined certificate chain");
										showMsgDialog(
												R.string.checkSettingsCertificateChainDeclinedTitle,
	...
	private void onClickCheckSettings()
	{
		String title = getResources().getString(
				R.string.checkSettingsProgressDialogTitle);
		String msg = getResources().getString(
				R.string.checkSettingsProgressDialogMessage);
		mProgDialog = ProgressDialog.show(this, title, msg, true, // no time
																	// limit
